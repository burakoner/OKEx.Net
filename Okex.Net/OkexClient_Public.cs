...
namespace Okex.Net
{
    public partial class OkexClient
    {
        ...
        /// <param name="ct">Cancellation Token</param>
        /// <returns></returns>
        public virtual WebCallResult<OkexInterestRate> GetInterestRates(CancellationToken ct = default) 
        ...
        /// <param name="ct">Cancellation Token</param>
        /// <returns></returns>
        public virtual async Task<WebCallResult<OkexInterestRate>> GetInterestRatesAsync(CancellationToken ct = default)
        {
            var result = await UnifiedApi.ExecuteAsync<OkexRestApiResponse<IEnumerable<OkexInterestRate>>>(UnifiedApi.GetUri(Endpoints_V5_Public_InterestRateLoanQuota), HttpMethod.Get, ct).ConfigureAwait(false);
            if (!result.Success) return result.AsError<OkexInterestRate>(new OkexRestApiError(result.Error.Code, result.Error.Message, result.Error.Data));
            if (result.Data.ErrorCode > 0) return result.AsError<OkexInterestRate>(new OkexRestApiError(result.Data.ErrorCode, result.Data.ErrorMessage, null));
