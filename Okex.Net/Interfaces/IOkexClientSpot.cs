...
namespace Okex.Net.Interfaces
{
	public interface IOkexClientSpot
	{
		...
		/// <returns></returns>
		WebCallResult<IEnumerable<RestObjects.Spot.TradingPair>> Spot_GetTradingPairs(CancellationToken ct = default);
		/// <summary>
		/// This provides snapshots of market data and is publicly accessible without account authentication.
		/// Retrieves list of trading pairs, trading limit, and unit increment.
		/// Rate limit: 20 Requests per 2 seconds
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		Task<WebCallResult<IEnumerable<RestObjects.Spot.TradingPair>>> Spot_GetTradingPairs_Async(CancellationToken ct = default);
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		WebCallResult<RestObjects.Spot.OrderBook> Spot_GetOrderBook(string symbol, int? size = null, decimal? depth = null, CancellationToken ct = default);
		/// <summary>
		/// Retrieve a trading pair's order book. Pagination is not supported here; the entire orderbook will be returned per request. This is publicly accessible without account authentication. WebSocket is recommended here.
		/// Rate limit: 20 requests per 2 seconds
		/// </summary>
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		Task<WebCallResult<RestObjects.Spot.OrderBook>> Spot_GetOrderBook_Async(string symbol, int? size = null, decimal? depth = null, CancellationToken ct = default);
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		WebCallResult<IEnumerable<RestObjects.Spot.Ticker>> Spot_GetAllTickers(CancellationToken ct = default);
		/// <summary>
		/// Retrieve the latest price snapshot, best bid/ask price, and trading volume in the last 24 hours for all trading pairs. This is publicly accessible without account authentication.
		/// Rate limit: 20 requests per 2 seconds
		/// </summary>
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		Task<WebCallResult<IEnumerable<RestObjects.Spot.Ticker>>> Spot_GetAllTickers_Async(CancellationToken ct = default);
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		WebCallResult<RestObjects.Spot.Ticker> Spot_GetSymbolTicker(string symbol, CancellationToken ct = default);
		/// <summary>
		/// Retrieve the latest price snapshot, best bid/ask price, and trading volume in the last 24 hours for a particular trading pair. This is publicly accessible without account authentication.
		/// Rate limit: 20 requests per 2 seconds
		/// </summary>
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		Task<WebCallResult<RestObjects.Spot.Ticker>> Spot_GetSymbolTicker_Async(string symbol, CancellationToken ct = default);
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		WebCallResult<IEnumerable<RestObjects.Spot.Trade>> Spot_GetTrades(string symbol, int limit = 60, CancellationToken ct = default);
		/// <summary>
		/// Retrieve the latest 60 transactions of all trading pairs.
		/// Rate limit: 20 requests per 2 seconds
		/// </summary>
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		Task<WebCallResult<IEnumerable<RestObjects.Spot.Trade>>> Spot_GetTrades_Async(string symbol, int limit = 60, CancellationToken ct = default);
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		WebCallResult<IEnumerable<RestObjects.Spot.Candle>> Spot_GetCandles(string symbol, SpotPeriod period, DateTime? start = null, DateTime? end = null, CancellationToken ct = default);
		/// <summary>
		/// Retrieve the candlestick charts of the trading pairs. This API can retrieve the latest 2000 entries of data. Candlesticks are returned in groups based on requested granularity. Maximum of 2,000 entries can be retrieved.
		/// Rate limit: 20 requests per 2 seconds
		/// </summary>
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		Task<WebCallResult<IEnumerable<RestObjects.Spot.Candle>>> Spot_GetCandles_Async(string symbol, SpotPeriod period, DateTime? start = null, DateTime? end = null, CancellationToken ct = default);
		#endregion
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		WebCallResult<IEnumerable<RestObjects.Spot.Account>> Spot_GetAllBalances(CancellationToken ct = default);
		/// <summary>
		/// This retrieves the list of assets, (with non-zero balance), remaining balance, and amount available in the spot trading account.
		/// Rate limit: 20 requests per 2 seconds
		/// </summary>
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		Task<WebCallResult<IEnumerable<RestObjects.Spot.Account>>> Spot_GetAllBalances_Async(CancellationToken ct = default);
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		WebCallResult<RestObjects.Spot.Account> Spot_GetSymbolBalance(string currency, CancellationToken ct = default);
		/// <summary>
		/// This retrieves information for a single currency in your account, including the remaining balance, and the amount available or on hold.
		/// Rate Limit: 20 requests per 2 seconds
		/// </summary>
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		Task<WebCallResult<RestObjects.Spot.Account>> Spot_GetSymbolBalance_Async(string currency, CancellationToken ct = default);
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		WebCallResult<IEnumerable<RestObjects.Spot.Bill>> Spot_GetSymbolBills(string currency, SpotBillType? type = null, int limit = 100, int? before = null, int? after = null, CancellationToken ct = default);
		/// <summary>
		/// This retrieves the spot account bills dating back the past 3 months. Pagination is supported and the response is sorted with most recent first in reverse chronological order.
		/// Rate Limit: 20 requests per 2 seconds
		/// <param name="currency">The token symbol, e.g. 'BTC'. Complete account statement for will be returned if the field is left blank</param>
		...
		/// <param name="after">Pagination of data to return records earlier than the requested ledger_id</param>
		/// <param name="ct">Cancellation Token</param>
		Task<WebCallResult<IEnumerable<RestObjects.Spot.Bill>>> Spot_GetSymbolBills_Async(string currency, SpotBillType? type = null, int limit = 100, int? before = null, int? after = null, CancellationToken ct = default);
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		WebCallResult<RestObjects.Spot.NewOrder> Spot_PlaceOrder(string symbol, SpotOrderSide side, SpotOrderType type, SpotTimeInForce timeInForce = SpotTimeInForce.NormalOrder, decimal? price = null, decimal? size = null, decimal? notional = null, string? clientOrderId = null, CancellationToken ct = default);
		/// <summary>
		/// OKEx spot trading supports only limit and market orders. More order types will become available in the future. You can place an order only if you have enough funds.
		/// Once your order is placed, the amount will be put on hold until the order is executed.
		/// Rate limit: 100 requests per 2 seconds
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		Task<WebCallResult<RestObjects.Spot.NewOrder>> Spot_PlaceOrder_Async(string symbol, SpotOrderSide side, SpotOrderType type, SpotTimeInForce timeInForce = SpotTimeInForce.NormalOrder, decimal? price = null, decimal? size = null, decimal? notional = null, string? clientOrderId = null, CancellationToken ct = default);
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		WebCallResult<RestObjects.Spot.NewOrder> Spot_CancelOrder(string symbol, long? orderId = null, string? clientOrderId = null, CancellationToken ct = default);
		/// <summary>
		/// This is used to cancel an unfilled order.
		/// Rate limit: 100 requests per 2 seconds
		/// </summary>
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		Task<WebCallResult<RestObjects.Spot.NewOrder>> Spot_CancelOrder_Async(string symbol, long? orderId = null, string? clientOrderId = null, CancellationToken ct = default);
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		WebCallResult<IEnumerable<RestObjects.Spot.OrderDetails>> Spot_GetAllOrders(string symbol, SpotOrderState state, int limit = 100, int? before = null, int? after = null, CancellationToken ct = default);
		/// <summary>
		/// This retrieves the list of your orders from the most recent 3 months. This request supports paging and is stored according to the order time in chronological order from latest to earliest.
		/// Rate limit: 20 requests per 2 seconds
		/// </summary>
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		Task<WebCallResult<IEnumerable<RestObjects.Spot.OrderDetails>>> Spot_GetAllOrders_Async(string symbol, SpotOrderState state, int limit = 100, int? before = null, int? after = null, CancellationToken ct = default);
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		WebCallResult<IEnumerable<RestObjects.Spot.OrderDetails>> Spot_GetOpenOrders(string symbol, int limit = 100, int? before = null, int? after = null, CancellationToken ct = default);
		/// <summary>
		/// This retrieves the list of your current open orders. Pagination is supported and the response is sorted with most recent first in reverse chronological order.
		/// </summary>
		/// <param name="symbol">Trading pair symbol</param>
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		Task<WebCallResult<IEnumerable<RestObjects.Spot.OrderDetails>>> Spot_GetOpenOrders_Async(string symbol, int limit = 100, int? before = null, int? after = null, CancellationToken ct = default);
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		WebCallResult<RestObjects.Spot.OrderDetails> Spot_GetOrderDetails(string symbol, long? orderId = null, string? clientOrderId = null, CancellationToken ct = default);
		/// <summary>
		/// Retrieve order details by order ID.Can get order information for nearly 3 months。 Unfilled orders will be kept in record for only two hours after it is canceled.
		/// Rate limit: 20 requests per 2 seconds
		/// </summary>
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		Task<WebCallResult<RestObjects.Spot.OrderDetails>> Spot_GetOrderDetails_Async(string symbol, long? orderId = null, string? clientOrderId = null, CancellationToken ct = default);
		...
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		WebCallResult<RestObjects.Spot.TradeFeeRates> Spot_TradeFeeRates(CancellationToken ct = default);
		/// <summary>
		/// Obtain the transaction fee rate corresponding to your current account transaction level. The sub-account rate under the parent account is the same as the parent account. Update every day at 0am
		/// Rate limit: 1 requests per 10 seconds
		/// </summary>
		/// <param name="ct">Cancellation Token</param>
		/// <returns></returns>
		Task<WebCallResult<RestObjects.Spot.TradeFeeRates>> Spot_TradeFeeRates_Async(CancellationToken ct = default);
